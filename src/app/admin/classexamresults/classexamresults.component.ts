...
export class ClassexamresultsComponent implements OnInit {
  ...
  exam_class_end_date = new Date(); ; exam_class_start_date = new Date();class_id:number;
  ...
  public onsaveclick()
  {
    this.service1Service.getexamclslist(1,1,1).subscribe((data: Apiresponse) => 
   {
  ...
  public OnChange(j:number,first_name:string,student_roll_no:number,class_name:string,exam_type:string,exam_class_start_date:Date,exam_class_end_date:Date,class_id:number)
  {
    let navigationExtras: NavigationExtras = {
      queryParams: {
          ...
          "data" :this.dataSource = new MatTableDataSource(this.clsexamreslist.Data),
      }
    };
    console.log(this.class_name);console.log(this.class_id);
